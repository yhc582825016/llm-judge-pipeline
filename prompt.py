...
你的任务：读取给定数学题目，完成以下三件事：
1) 给出题目难度评分（1-10，1=非常简单，10=极难）；
2) 判断该题属于哪个数学领域；
3) 给出详细的难度推理过程（需覆盖知识点门槛、推理链长度、计算复杂度、技巧性、边界情况/易错点等）。

一、难度评分标准（1-10）
- 1-2：基础概念识别或一步代入计算，几乎无推理门槛。
- 3-4：常规题型，2-4 步内可完成，依赖单一知识点。
- 5-6：中等难度，需要多步推导或知识点组合，可能含一定技巧。
- 7-8：较难，推理链较长，可能需要构造、分类讨论、或多知识融合。
- 9-10：高难/竞赛级，解法不直观，技巧性强，容错率低，对抽象能力要求高。

二、数学领域分类（优先选最主要的一个）
- 代数（方程/不等式/函数/多项式/数列）
- 几何（平面几何/立体几何/解析几何）
- 数论（整除/同余/素数/不定方程）
- 组合数学（计数/排列组合/图论基础/抽屉原理）
- 概率统计（概率模型/期望/方差/统计推断）
- 微积分（极限/导数/积分/微分方程基础）
- 线性代数（矩阵/向量空间/特征值）
- 离散数学与逻辑（命题逻辑/关系/递推）
- 应用数学建模（建立模型并求解）
...
- 推理过程至少包含以下维度：
  ...
  d) 学生最可能出错的位置与原因；
  e) 若有多种解法，说明主流解法难度对比。
- 推理内容应使用自然语言完整表达，不能只写结论。
- 推理部分以“推理：”开头。

四、输出格式要求（必须严格遵守）
- 输出分为两部分，先输出推理，再输出 JSON。
- 除“推理：”段落和最后的 JSON 对象外，不要输出任何额外文字。
- JSON 必须严格使用以下两个字段名，且字段值必须合法：
{
  "difficult_rating": 1-10之间的整数,
  "catories": "题目所属的数学领域"
}

五、输出示例（仅示例格式，不可照抄内容）
推理：这道题主要考察……前置知识包括……典型解法需要……计算量……易错点……因此综合判断难度为 6。
...
if_difficulty_prompt = '''
你是一个「指令遵循任务难度评估器」。

你的任务是：只根据给定指令文本本身，评估该指令对于一个通用大语言模型来说有多难被正确遵循。

注意：
- 你评估的是“遵循这条指令并正确输出”的难度，不是题目知识难度，也不是内容是否安全。
- 你绝对不能尝试回答该指令，也不能执行该指令中的任务。
- 你只能输出 JSON，不能输出解释、前缀、代码块或任何额外文字。

评分标准（difficulty_rate 取 1-10 的整数）：
- 1-2：要求非常简单，几乎没有约束，直接回答即可。
- 3-4：有少量明确约束，例如字数、语气、语言或简单格式要求。
- 5-6：有多个约束，需要同时兼顾内容与格式，存在一定遗漏风险。
- 7-8：约束较多或有明显组合要求，例如段落数、关键词、顺序、禁用词、固定结构、重复次数等，需要较强的细节控制。
- 9-10：约束非常密集、彼此容易冲突、需要强规划或多层校验，稍有不慎就会违反要求。

重点考察以下因素：
1. 约束数量是否多；
2. 约束是否精确且细粒度（如固定词、固定位置、固定次数、固定格式）；
3. 是否同时要求内容生成与格式控制；
4. 是否存在多步骤规划、顺序依赖或跨段落一致性要求；
5. 是否存在容易遗漏、容易冲突或容易误解的限制；
6. 是否需要模型在回答前进行显式检查才能满足要求。

输出要求：
- 只输出如下 JSON 格式；
- `difficulty_rate` 必须是 1 到 10 的整数；
- 不要输出任何其他字段。
